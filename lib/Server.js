...
module.exports = _.class({
	...
	constructor: function(argv) {
		...
		if(totalAllocSizeGiB < 8 * _.GiB) this.fatal("Allocated space is too small. Size should be more then 8GiB");
		var allocSize = 0;
		var usedSize = 0;
		var initSegment = function(name) {
			this.segments = this.segments || {};
			if(allocSize < totalAllocSizeGiB && !this.segments[name]) {
				var storageClass = storageClasses[name.substr(0,1)];
				var segment = new storageClass(name, this);
				this.segments[name] = segment;
				allocSize += segment.getAllocatedSize();
				usedSize += segment.usedSize;
				this.log(' - init segment <'+name+'>');
			}
		}.bind(this);

		// choose selected segments
		if(options.segments !== undefined) {
			options.segments.split(',').forEach(function(segmentName){
				if(/^[DFNP][0-7]*$/.test(segmentName))
					initSegment(segmentName);
			});
		}
		...
		//while(allocSize < totalAllocSizeGiB) {
	...
	onWebRequest: function(request, response) {
		...
		//---- add header ------
		if(path.substr(0, 3) === '/-/') {
			//todo: check Origin. access only for base
	...
	parseNodeHeader: function(conn) {
		...
		//if(node && node.host && node.port && conn._req.connection.remoteAddress == node.host) // check real IP ??
	...
	requestToNode: function(op) {
		...
		var self = this, queue = [], fFinish, strm, response, lines = '', started;
		...
		function fnNext(err) {
			...
			if(fFinish && !queue.length) return onFinish && onFinish.call(self); // success finish
			try {
				if(onData && queue.length) onData.call(self, started = queue.shift());
			} catch (e) {
				fnAbort(e);
			}
		}
		function processLine(line) {
			...
			queue.push(pack);
			if(!fAsync || !started) fnNext();
		}
		var node = this.parseNodeId(nid);
		if(!node) return onError('Bad nid');
		var headers = {
			host: '-',
			...
			switch(op.type) {
				case 'json':
					strm.on("data", function(chunk) {
						...
						processLine(lines);
						fFinish = true;
						if(!fAsync || !started) fnNext(); // finish
					});
					break;
