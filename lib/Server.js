...
module.exports = _.class({
	...
	constructor: function(argv) {
		...
		this.log('opts', JSON.stringify(options, 0, 4));
	...
	getNodesBySegment: function(segmentId, count) {
		var nodes = (this.nodesBySeg[segmentId] || []).slice(),
			res;
	...
	addNode: function(node) {
		...
		var segments = String(node.seg||'')
			.split(',')
			.filter(function(seg){ return regexps.segmentId.test(seg) });
