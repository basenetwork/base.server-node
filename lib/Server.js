...
module.exports = _.class({
    ...
    constructor: function(argv) {
        ...
        if(!host) {
            host = this.getExternalInterface();
        }
        ...
        if(this.totalAllocSizeGiB < minSegSizeGiB * _.GiB) this.fatal("Allocated space is too small. Size should be more than " + minSegSizeGiB + "iGiB");
    ...
    initNodesInfo: function() {
        ...
        this.loadNodesInfo() || this.fatal("Can`t find any nodes");
    },

    loadNodesInfo: function () {
        return this.nodesInfoLocations.some(function (nodesFilePath) {
            return this.loadNodesInfoFromFile(nodesFilePath);
        }, this);
    },

    loadNodesInfoFromFile: function(filePath) {
        try {
    ...
    removeNode: function(nid, err) {
        if(this.nodes[nid]) {
            (this.nodes[nid].seg || '').split(',').forEach(function(segmentId) {
    ...
    startFSyncNodesInfo: function(start) {
        if(this.isTestMode()) return;

        var nextTry = this.startFSyncNodesInfo.bind(this),
    ...
    startScanNodes: function() {
        var nid = this.newNodes.anyKey() || _.randomKey(this.nodes),
            nextTry = this.startScanNodes.bind(this),
            delayNextTry = function () {
                setTimeout(nextTry, 3001);
            };
        ...
        if(!nid || nid == this.nid) {
            ...
            this.scanNodes(nid, delayNextTry);
        }
    },

    scanNodes: function (nid, onFinish) {
        this.requestToNode({
            nid: nid,
            path: '/-/nodes',
            type: 'json',
