...
var allAvailableSegmentTypes = (function () {
    var segmentTypes = [];
    var isSegmentTypeAdded = {};

    for (var ring_type in availableSegmentTypes) {
        (availableSegmentTypes[ring_type] || []).forEach(function(segmentType) {
            if (!isSegmentTypeAdded[segmentType]) {
                segmentTypes.push(segmentType);
                isSegmentTypeAdded[segmentType] = true;
            }
        });
    }
...
console.log('seg pattern' + segmentIdPattern);
var segmentIdRe = new RegExp('^' + segmentIdPattern);

var regexps = {
    segmentId: segmentIdRe,
    ...
    constructor: function(argv) {
        ...
        if(options.segments !== undefined) {
            options.segments.split(',').forEach(function(seg){
                if(this._isValidSegment(seg)) this.initSegment(seg);
            }, this);
        }
