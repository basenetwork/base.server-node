...
module.exports = _.class({
    ...
    constructor: function(argv) {
        ...
        // periodically refresh map of network
        setInterval(this.refreshNodesInfo.bind(this), 4713);

        // periodically flush nodes-info to disk
        this.saveNodesInfo();
        setInterval(this.saveNodesInfo.bind(this), 3559);
    },
    ...
    requestToNode: function(op) {
        ...
        function fnAbort(err) {
            ...
            if(self.options.debug) {
                self.debug('FAIL: Request to ' + nid + ' ' + op.path + ' aborted! Error: ' + err);
            }
            self.removeNode(nid);

            // add node to error-nodes-list
            self.nodesErrors.set(nid, +new Date() + (1 + Math.random()) * 120e3);
        ...
        if(this.options.debug) {
            this.debug("out HTTP GET "+node.host+":"+node.port+" "+op.path+" headers: "+JSON.stringify(headers));
        }
    ...
    onWebRequest: function(request, response) {
        ...
        // --- out debug info ---
        if(this.options.debug) {
            this.debug("in  "+conn.method+" "+path+"  headers: "+JSON.stringify(request.headers));
        }
    ...
    httpDebug: function(conn) {
        if(this.options.debug) {
            conn.response(200, JSON.stringify({
                Version:    this.version,
                NodeID:     this.nid,
                Options:    this.options,
                Allocated:  this.allocSize,
                TotalAlloc: this.totalAllocSizeGiB,
                NodesNew:   this.newNodes.data,
                NodesError: this.nodesErrors.data,
                Nodes:      this.nodes
            }));
        } else {
            conn.response(200, "");
    ...
    saveNodesInfo: function() {
        ...
        if(!this._fChangedNodesInfo) return;
    ...
    refreshNodesInfo: function() {
        var nid;
        // select random error-node
        if(Math.random() < 0.10) {
            nid = this.nodesErrors.anyKey();
            if(nid) this.nodesErrors.unset(nid)
        }
        // select random node from newNodes list
        if(!nid) {
            nid = this.newNodes.anyKey();
            if(nid) this.newNodes.unset(nid);
        }
        // or select random active node
        nid = nid || _.randomKey(this.nodes);
        if(!nid || nid == this.nid) {
            return;
        }
        this.requestToNode({
