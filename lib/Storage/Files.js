...
module.exports = _.class(Storage, {
	...
	httpCommandData: function(conn) {
		...
		this._sql.selectData.get({
			...
			conn.setResponseHeader('Content-Length', row.len);
            conn.setExpire();
            ...
            var rs = fs.createReadStream(this.dir + '/storage.dat', { start:row.pos, end:row.pos + row.len-1 });
            rs = rs.pipe(zlib.createGunzip());
            conn.response(200, rs);
		}.bind(this));
	}
