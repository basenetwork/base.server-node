...
module.exports = {
    ...
    b64ToStr: function(base64, enc) {
        try {
            return new Buffer(base64, "base64").toString(enc);
        } catch(e) {
            return "";
        }
    },
    ...
    sha256: function(data, encoding, outEncoding) {
        return crypto.createHash("sha256").update(data, encoding || "utf8").digest(outEncoding || "hex");
    },
    ...
    compareIP: function(ip1, ip2) {
        if(ip1.indexOf(':') < 0 ^ ip2.indexOf(':') < 0) return ip1.indexOf(':') >= 0 ? 1 : -1;
    ...
    cache: function(maxLength) {
        return {
            ...
            anyKey: function() {
                for(var i in this.data) return i;
            },
            get: function(key) {
